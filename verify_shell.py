...
def run_tests():
    ...
    # Just calling it to ensure no crash, deep inspection difficult via raw call
    ...
    # Mock query params
    req_bad.GET = type('QueryDict', (object,), {'get': lambda k, d=None: 'DEV-2'})
    # Need to manually attach query params correctly if RequestFactory didn't do it
    # But let's use the factory way:
    factory = RequestFactory()
    req_bad = factory.get('/?device_id=DEV-2')
    middleware = SessionMiddleware(lambda x: None)
    middleware.process_request(req_bad)
    req_bad.session['user_id'] = u1.User_ID
    req_bad.session['role'] = u1.Role
    req_bad.session.save()
    ...
    elif response.data and "access denied" in str(response.data.get('error','')).lower():
